...
author: "Ignacio Almodóvar, Luis Ángel Rodriguez Garcia"
date: "2/9/2022"
output: pdf_document
---
...
Let's look at the following plots which represent for each type of drug the likelihood depending on the values of the quantitative variables *Age* and *Na_to_K*:

```{r echo = FALSE, include=TRUE, fig.width=7}
quantitative.vars <- drugs %>% dplyr::select(where(is.numeric))
mms=apply(quantitative.vars, 
          MARGIN=2, 
          function(x) unlist(by(x,drugs$Drug,mean)))
sds=apply(quantitative.vars, 
          MARGIN=2, 
          function(x) unlist(by(x,drugs$Drug,sd)))
...
for(i in 1:2){
  ...
  if(i==1)
    legend(x="topleft", legend=levels(factor(drugs$Drug)), fill = c(1:5))
  else
    legend(x="topright", legend=levels(factor(drugs$Drug)), fill = c(1:5))
}

```

As we can see above in the first plot, for people at the age of 65 the more likely drug associated is the B one meanwhile for people around 35 the more likely drug associated is the A one. In addition, it looks like the drug C, X and Y have the same likelihood based on the quantitative variable *Age*.

In the second plot, we can see that for a value around 22.5 of sodium-to-potassium we get in all the cases the drug Y. Therefore, this variable is so informative for classifying the drug Y. For a value around 12.5 we can see that the likelihood of getting some drug A, B, C, or X is the same.


## Prediction

Now that we have trained the model, we can predict the values using the test dataset. Let's observed what happens for example with the rows 1 and 7:

```{r predictions17, echo = FALSE, include=TRUE}
header = names(predict(model,drugs[trainSet,],type="raw")[1,])
row1 = round(predict(model,drugs[trainSet,],type="raw")[1,], digits=2)
row7 = round(predict(model,drugs[trainSet,],type="raw")[7,], digits=2)
...
We can see in Table \ref{tab:predictions17} that the prediction for the first observation on the test set is 100% associated with drug Y. For the seventh observation, we can see that the prediction classifies the data as drug C with 77% of probability, so it is the most likely drug for that patient.
